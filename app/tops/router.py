...
async def get_top(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    top_case_luck = await db.scalars(
        select(User_model)
        .order_by(desc(
            User_model.successful_cases_cnt /
            (User_model.case_opened + 1))
        )
        .limit(10)
    )
    top_upgrades_luck = await db.scalars(select(User_model).order_by(desc(User_model.successful_upgrades_cnt / (User_model.upgrades_cnt + 1))).limit(10))
    ...
    if user:
        return templates.TemplateResponse('tops.html', {'request': request,
                                                        ...
                                                        'top_upgrades_luck_dict': top_upgrades_luck_dict})
