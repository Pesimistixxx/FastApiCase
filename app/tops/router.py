...
async def get_top(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    top_case_luck = await db.scalars(select(User_model).where(User_model.case_opened >= 100).order_by(desc(User_model.successful_cases_cnt / (User_model.case_opened + 1))).limit(10))
    top_upgrades_luck = await db.scalars(select(User_model).where(User_model.upgrades_cnt >= 50).order_by(desc(User_model.successful_upgrades_cnt / (User_model.upgrades_cnt + 1))).limit(10))
    top_contracts_luck = await db.scalars(select(User_model).where(User_model.contracts_cnt >= 50).order_by(desc(User_model.successful_contracts_cnt / (User_model.contracts_cnt + 1))).limit(10))
    ...
    top_case_luck_dict = {user.username: f'{user.successful_cases_cnt / (user.case_opened + 1) * 100:.2f}' for user in top_case_luck}
    top_upgrades_luck_dict = {user.username: f'{user.successful_upgrades_cnt / (user.upgrades_cnt + 1) * 100:.2f}' for user in top_upgrades_luck}
    top_contracts_luck_dict = {user.username: f'{user.successful_contracts_cnt / (user.contracts_cnt + 1) * 100:.2f}' for user in top_contracts_luck}
    if user:
